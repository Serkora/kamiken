...
- Добавить периодический пинг игроков (Сложно, требует полной переработки сервера
в асинхронный, с эвентлупом и т.д.)
		
#### Update log
##### Asyncio Server 0.1.0
- Всё переписано под asyncio.
- Переработана вся логика сервера: если списки игроков и игр, по (нескольким) заголовкам 
в сообещниях совершаются нужные действия. Поддерживается реконнект в случае временной
пропажи соединения или изменения айпи/порта (если клиент не перезапускался). 
-Играм присваиваются id и в словарь записываются игроки, таким образом достигается
возможность параллельных игр.
- Есть лобби.
- Клиент под это всё пока не написан вообще.

##### V 0.3.1s
- Графика полностью оптимизирована. В начале игры создаётся необходимое количество
объектов-спрайтов, которые далее, по мере необходимости, изменяются.
- Добавлен пропуск хода. Присутствует баг: так как fade_stone создаётся один раз,
в начале игры, смена игрока (по shift-q) позволит походить, но рисуемый при движении
мышкой полупрозрачный камень всё равно будет того цвета, которым была начата игра. 
Конечно, в реальной игре никакой смены игрока по shift-q нет, так что не так страшно.
- Есть рестарт игры, но пока не будет прикручен message\_classifier, по сети
передаваться ничего не будет (и так уже скип на (-1,-1), начало игры на player = 0,
пора остановиться костыли и неясные решения делать).
