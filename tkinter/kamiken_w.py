...
PART2. Allow different gameplay settings (currently everything is hardcoded)
...
DONE5. Improve widget placement and size to allow the main window to be properly resizable
...
NOPE8. Nicknames in chat, maybe?
NOPE9. Extend Player class (currently only player number is used)
...
NOPE16. Possibly implement a dictionary for ishi placement storage
		—————————— Nah, lists work fine
DONE17. Turn indication
NOPE19. Server storing last N messages —————— No need to.
...
Bugs:
	...
	2. Can't change client's listener ip address ——————— Don't event need to bind client's
														...
														it works as it is.
										——————— After implementing packet transmission,
												had to put it back again. And it hadn't
												worked on windows anyway. IP change is done
	...
	5. 'switchplayers' command is not really working.
	6. Something's wrong with chat: 
...
class Geoffrey(object):
	def __init__(self,player=2):
		self.player=player			# set player number. default is 2
		self.makemove='no'
		self.stop='no'				# if 'yes' — doesn't wait for his move anymore, stops
		self.halt='no'
		
	def rise(self):
		printv("geoffrey rise")		# start waiting for turn loop. This is not in __init__ 
		self.wait_for_board()		# just to properly deal with threading.
		self.running()
		printv("geoffrey sleep")
	...
	def wait_for_board(self):
		while self.makemove!='yes':
			time.sleep(1)
		self.all_moves=[[x+0.5,y-0.5] for x in range(0,goban.size) for y in range(1,goban.size+1)]
		self.valid_moves=self.all_moves.copy()
	
	def wait_for_turn(self):
#		print("geoffrey wait")
		while True:
			if self.halt=='yes': return
			if self.stop=="yes" and self.player==1:
				goban.count_score()
				client.update_text("WHITE: "+str(goban.score[0])+" BLACK: "+str(goban.score[1]))
				client.restart_ai()					# Auto-restart at the end of the
				geoffrey.stop='no'					# game with two AI players.
				geoffrey2.stop='no'
				break
			elif self.stop=="yes":
				goban.turn=goban.turn*2%3			# in order not to loop-restart,
				time.sleep(1)						# only one AI player restarts the game
			if self.player==goban.turn:
				self.geoffrey_move()				# if AI's turn — make a move
			t=random.random()/5
			time.sleep(t)

	def valid_moves_gen(self,player):
		if player==1: hitsOP,ishiOP,ishiSLF=goban.hitsB,goban.ishiB,goban.ishiA
		if player==2: hitsOP,ishiOP,ishiSLF=goban.hitsA,goban.ishiA,goban.ishiB
		for move in self.valid_moves[:]:
			if move in hitsOP or move in ishiOP or move in ishiSLF: 
				self.valid_moves.remove(move)
	
	def geoffrey_move(self):
#		print("geoffrey stone place")
		self.valid_moves_gen(self.player)
		while True:
			print(len(self.valid_moves))
			m=1								
			override=False					# if True — skips the "already beat by me" check
			move_matrix,free=self.create_matrix()		# create a matrix of the board
			movet, movef=self.three_or_four(move_matrix,self.player) # move that beat 3+ hits
			movetr=self.trade_off(move_matrix,self.player)	# trade-off. -1 to yourself, -2+ to opponent
			if self.player==1: hitsSLF=goban.hitsA		# needed for the "beat by me" check
			elif self.player==2: hitsSLF=goban.hitsB
			if len(movef)>0:
				print("beating four")
				ind=random.randrange(0,len(movef))	# if found any moves that can beat
				colint=movef[ind][0]				# four of the opponents hits
				row=movef[ind][1]					# than it becomes the move
			elif len(movet)>0:
				print("beating three")
				ind=random.randrange(0,len(movet))	# if can beat 3 of the opponents hits
				colint=movet[ind][0]				# that it is the second best move
				row=movet[ind][1]
			elif len(movetr)>0 and free<5:
				print("trading off")
				ind=random.randrange(0,len(movetr))	# or found any trade-offs and less
				colint=movetr[ind][0]				# than 5 empty squares on board
				row=movetr[ind][1]					
				override=True			
			else:
				while True:
					mvnm=random.randrange(0,len(self.valid_moves))
					mv=self.valid_moves[mvnm]
					colint=int(mv[0])
					row=int(mv[1]+1)
					m+=1
					if mv not in hitsSLF: break
					elif m==50: self.stop='yes'; return
			col=string.ascii_lowercase[colint]
	...
	def create_matrix(self):
		"""
		Creates a current matrix with all the stones and hit squares.
		0 — free
		1 — hits by player 1
		2 — hits by player 2
		3 — stones of player 1
		4 — stones of player 2
		5 — hits by both.
		"""
		matrix=np.zeros(shape=(goban.size,goban.size))
		for i in range(1,len(goban.hitsA)):		# for all player 1 hits
			if any(x<0 for x in goban.hitsA[i]): continue
			if any(x>=goban.size for x in goban.hitsA[i]): continue
			matrix[-int(goban.hitsA[i][1])-1,int(goban.hitsA[i][0])]=1
		for i in range(1,len(goban.hitsB)):		# for all player 2 hits
			if any(x<0 for x in goban.hitsB[i]): continue
			if any(x>=goban.size for x in goban.hitsB[i]): continue
			if goban.hitsB[i] in goban.hitsA:	# checks if it also hit by player 1
				matrix[-int(goban.hitsB[i][1])-1,int(goban.hitsB[i][0])]=5			
			else:
				matrix[-int(goban.hitsB[i][1])-1,int(goban.hitsB[i][0])]=2
		for i in range(0,len(goban.ishiA)):		# for all player 1 stones
			matrix[-int(goban.ishiA[i][1])-1,int(goban.ishiA[i][0])]=3
		for i in range(0,len(goban.ishiB)):		# for all player 2 stones
			matrix[-int(goban.ishiB[i][1])-1,int(goban.ishiB[i][0])]=4
		free=len(np.where(matrix==0)[1])		# number of free squares
		return matrix, free
	
	def three_or_four(self,matrix,pl):
		"""
		Iterate through each element. (Try/except are to allow for border rows and columns.)
		If it is zero, check what's around. If there are 3 squares beat by the opponent,
		then add coords of that square to 'movet' (movethree) list. If 4 — 'movef' (movefour).
		'*2%3' operation makes 2->1 and 1->2.
		"""
		z=0
		movet=[]
		movef=[]
		pl=pl*2%3								# search for not your hits
		for m in range(0,len(matrix)):
			for n in range(0,len(matrix)):
				if matrix[m,n]==0:				# if square is empty
					try: 
						if matrix[m,n+1]==pl: z+=1
					except: pass 
					try:
						if matrix[m,n-1]==pl: z+=1
					except: pass
					try:
						if matrix[m+1,n]==pl: z+=1
					except: pass
					try:
						if matrix[m-1,n]==pl: z+=1
					except: pass
					if z==3: 
						movet.append([n,goban.size-(m)])
					if z==4: 
						movef.append([n,goban.size-(m)])
				z=0
		return movet,movef
						
	def trade_off(self,matrix,pl):
		"""
		The same as 'three_or_four', but check all the squares hit by yourself and
		check if there are 2 or more of the opponent's hits
		"""
		z=0
		movetr=[]
		pl=pl*2%3								# search for not your hits
		for m in range(0,len(matrix)):
			for n in range(0,len(matrix)):
				if matrix[m,n]==pl*2%3:			# if square is hit by you
					try: 
						if matrix[m,n+1]==pl: z+=1
					except: pass 
					try:
...
class Board(object):
	def __init__(self):
		self.sizedef=10
		self.size=self.sizedef
		self.komi=0.5
		self.turndef=1
		self.turn=self.turndef
	...
	def validate(self,ishi,player):
		...
		if player==1:
			if ishi[0] in self.ishiA:
				if self.mode=='AI' and character.player!=goban.turn:
					pass
				else:
					pass
...
#					client.update_text("You've already placed a stone there!")
	...
	def count_score(self, ai='no', ishiA='', hitsA='', ishiB='', hitsB=''):
		if ai=='no':
			ishiA,hitsA=self.ishiA,self.hitsA
			ishiB,hitsB=self.ishiB,self.hitsB			
		
		indA=[hitsA[i] not in ishiA for i in range(0,len(hitsA))]
		htA=[hitsA[i] for i in [index for index,value in enumerate(indA) if value]]
		htA=np.array(htA)
		a=htA[:,0]>0
		b=htA[:,0]<=self.size
		c=htA[:,1]>0
		...
		indB=[hitsB[i] not in ishiB for i in range(0,len(hitsB))]
		htB=[hitsB[i] for i in [index for index,value in enumerate(indB) if value]]
		htB=np.array(htB)
		a=htB[:,0]>0
		b=htB[:,0]<=self.size
		c=htB[:,1]>0
		...
		if ai=='no':
			if self.turndef==1:
				self.score=[Ascore,Bscore+self.komi]
			elif self.turndef==2:
				self.score=[Ascore+self.komi,Bscore]
		else:
...
class GameClient(object):
	...
	def __init__(self, master):
		...
		self.conn = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # init socket
		self.conn.bind((self.myip,self.myport))		# bind it to the ip/port pair.
				# network listener
		...
		ThrListen.start()
				# Gameplay default values and board/player initialization
		global goban
		global character
		goban=Board()
		character=Player()
...
#		self.send('confirmpls','conr')
	...
	def create_base_elements_place(self):					# creates all base GUI elements
...
#		self.text.bind('<1>',self.undisable_text)
		...
		self.button_rules = Tk.Button(master=self.root, text='Rules',
									width='10', command=self.show_rules)
		self.button_rules.place(relx=.06, rely=.3, anchor='c')
		...
		self.button_testing.place(relx=.12, rely=.03, anchor='c')
	...
	def create_base_elements(self):					# creates all base GUI elements
		...
		self.canvas = FigureCanvasTkAgg(self.fig, master = self.root)
		self.canvas.get_tk_widget().grid(row=1, column=2, rowspan=7, sticky=Tk.S+Tk.N+Tk.W+Tk.E)
					
					# main text fied
		self.text=Tk.Text(master=self.root,height=8,width=40)
		self.text.grid(row=5, column=0, columnspan=2)#, sticky=Tk.W+Tk.S+Tk.N)
		self.text.insert(Tk.INSERT, "POEHALEE?\n")
		self.text.configure(state="disabled")
					
					# main entry
		self.MainInput = Tk.StringVar()
		self.MainEntry = Tk.Entry(master=self.root, textvariable=self.MainInput, width='30')
		self.MainEntry.grid(row=3, column=0, columnspan=2, sticky=Tk.N)
		self.MainEntry.bind("<Return>",self.on_key_event_MainEntry)	

					# chat
		self.Chat=Tk.scrolledtext.ScrolledText(master=self.root,height=15,width=35)
		self.Chat.grid(row=1, column=3, columnspan=2, rowspan=5, sticky=Tk.N+Tk.E)
		self.Chat.tag_configure("red",foreground="red")
		self.Chat.tag_configure("blue",foreground='blue')
		self.Chat.configure(state='disabled')
		self.ChatInput=Tk.StringVar()
		self.ChatEntry=Tk.Entry(master=self.root,textvariable=self.ChatInput,width=20)
		self.ChatEntry.grid(row=6, column=3, sticky=Tk.W+Tk.S)
		self.ChatEntry.bind('<Return>',self.on_key_event_ChatEntry)
		self.ChatClear = Tk.Button(master=self.root, text="clear",width='3', 
									command=self.clear_chat)
		self.ChatClear.grid(row=6, column=4)

					# Scoreboard and turn indicator
		self.turnindicator = Tk.Text(master=self.root, height=1,width='25')
		self.turnindicator.configure(background='black')
		self.turnindicator.grid(row=0, column=2)
		self.turnindicator.bind("<Enter>",lambda x:self.update_turnind('score'))	
		self.turnindicator.bind("<Leave>",self.update_turnind)	
		...
		self.button_settings = Tk.Button(master=self.root, text='Settings', 
									width='10', command=self.settings_menu)
		self.button_settings.grid(row=1, column=1)
		self.button_server = Tk.Button(master=self.root, text='Start server', 
									width='10', command=self.start_server)
		self.button_server.grid(row=0, column=1)
		self.button_server.bind('<Enter>',self.servertimedbutton)	# calls function to
		self.button_server.bind('<Leave>',self.servertimedbutton)	# wait 1 sec and change
		self.sercurs=['','']				# state/text if mouse is still over the button
		self.button_rules = Tk.Button(master=self.root, text='Rules',
									width='10', command=self.show_rules)
		self.button_rules.grid(row=2, column=0)
		self.button_AI = Tk.Button(master=self.root, text='Start AI game', 
									width='10', command=self.initAI)
		self.button_AI.grid(row=1, column=0)
#		self.button_testing = Tk.Button(master=self.root, text='Test button',
#									width='10', command=self.test_fun)
#		self.button_testing.place(relx=.12, rely=.03, anchor='c')
...
#			goban.mode=setting[4:]
	...
	def settings_menu(self):
		...
		def save_settings():
			...
			if self.listenip_var.get()!=self.myip:
				self.myp=self.litenip_var.get()
				self.listenerip = self.listenip_var.get()
				self.conn.close()
				self.conn=socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
				self.conn.bind(self.myip,self.myport)
			goban.sizedef=int(self.brd_var.get())
#			goban.size=int(self.brd_var.get())
...
#		self.window_settings.rowconfigure((0,1,2), weight=1)
		...
		self.listenip_inp.grid(row=1, column=3)									
		self.listenip_inp.bind('<KeyRelease>',self.on_key_event_Settings)	
							# board size
		self.brd_var = Tk.StringVar()					# entry with board size
		self.brd_var.set(goban.sizedef)		
		self.brd_inp = Tk.Entry(master=self.window_settings, 
									textvariable=self.brd_var, width='15')
		self.brd_inp.grid(row=2, column=3)									
		self.brd_inp.bind('<KeyRelease>',self.on_key_event_Settings)	
				# text description
		Tk.Label(master=self.window_settings,text="  ").grid(row=0, column=0)
		Tk.Label(master=self.window_settings,text="Server IP").grid(row=0, column=2)
		Tk.Label(master=self.window_settings,text="My/Listener IP").grid(row=1, column=2)
		Tk.Label(master=self.window_settings,text="Borad size").grid(row=2, column=2)
			
				# check if already connected and disabled inputs
		if self.connected=='yes':
			self.servip_inp.configure(state='disable')
			self.listenip_inp.configure(state='disable')
...
#		self.canvas.get_tk_widget().grid(row=5, column=2, rowspan=5)
	...
	def test_fun(self):
		...
		if s=="ai2":		
			...
			subprocess.call(('open','kamikenrules.pdf'))
		elif len(s)>0:
			time.sleep(int(s))
		self.MainInput.set('')
		"""
...
#		self.packets=False
	...
	def update_turnind(self,type='turn'):
		...
		if type=='score' or self.gamestate=="stopped":
			...
			self.turnindicator.insert("1.0","White:"+str(goban.score[0])+"    Black:"+str(goban.score[1]))
...
#			self.window_restart.destroy()				# close message window
	...
	def load_game(self,savegame):
		...
		for i in range(0,lB):
			...
			goban.store_ishi(xB,yB,2)				# and plot all of the stones so that
		goban.turn=self.savegame[2]					# they are surely on top
		goban.size=self.savegame[3]
		if savegame=='localAI': geoffrey.makemove='yes'
	...
	def save_game(self):
		self.savegame=[goban.ishiA,goban.ishiB,goban.turn,goban.size]	
		if self.mode=='internet':							# combines both your and
			f = open('saves/savegame','wb')					# opponent's stones plus
		elif self.mode=='AI':								# current turn and board size 
			f = open('saves/savegameAI','wb')				# and dumps them in a file
	...
	def show_rules(self):
		filepath='docs/kamikenrules.pdf'
	...
	def initAI(self,player=2):
		...
		geoffrey=Geoffrey(player)						# create AI
	...
	def initAI2(self,player):
		self.MainEntry.bind("<Return>", self.mainentry_AI)
		
		global geoffrey2								# make global variable
		geoffrey2=Geoffrey(player)						# create AI
		ThrGeof=Thread(target=geoffrey2.rise,args=())	# make AI thread
		ThrGeof.setDaemon(True)
		ThrGeof.start()									# It's ALIVE!
		geoffrey2.stop='no'
		geoffrey2.makemove='yes'
	...
	def start_AI(self):
		...
		geoffrey.makemove='yes'							# let geoffrey start his business
		self.update_turnind()
