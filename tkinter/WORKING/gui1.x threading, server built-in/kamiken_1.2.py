...
DONE	1.1 Check if the square already contains opponent's stone or has been hit by it
	...
	4. Automatic score counting
		4.1 and on-going scoreboard
	5. Restart only clears the board, self.turn stays the same — need fix (probably related
		to custom settings at startup)
	6. Improve widget placement and size to allow the main window to be properly resizable
DONE7. Integrate GameLauncher into GameClient — it's only 3 functions anyway.
	8. Update chat colouring to only search the beginning of the last added line
	9. Nicknames in chat, maybe?
	10. Extend Player class (currently only player number is used)
		possibly get rid of it, since everything happens on board anyway, and 
		player number can be kept in Client
PROG11. Add Board class
	12. Add saves, skips
	13. Add reset_board prompt
	14. Improve startup — why have start, create_board and berehit?
	15. Rethink the placing/hitting/validating strategy (i.e. get rid of [1,2].remove(player))
...
Bugs:
	...
	2. Can't change client's listener ip address ——————— Don't event need to bind client's
														...
														it works as it is.
	3. "You've already placed a stone there" prints twice.
...
class Board(object):
	...
	def validate(self,ishi,player):
		x=int(string.ascii_lowercase.index((ishi[0].lower())))
		y=int(ishi[1:])
	...
	def store_hits(self,hx,hy,player):
		if player==1:					# since hx and hy are lists,
			for x in hx:				# and this will probably be then used to plot
				self.hitsAx.append(hx)	# after loading game, for example,
			for y in hy:				# each value is appended separately
				self.hitsAy.append(hy)	# to have one (four) large list of marker values
		elif player==2:
			for x in hx:
				self.hitsBx.append(hx)
			for y in hy:
				self.hitsBy.append(hy)
	...
	def count_hits(self,x1,y1,x2,y2):
		pass
...
#		launcher.send('finrestart','settings')
	...
	def refresh_figure(self,x,y,hx,hy,player):
		if player==1:
			...
			return
		plt.scatter(x+0.5,y-0.5,c=colour[0],s=300)
		plt.scatter(hx,hy,c=colour[1],s=50)
	...
	def on_key_event_MainEntry(self,event):
		...
		if self.gamestate!="playing" or s=="stop" or s=="save" or s=="skip" or s=="syshalt":
		...
		elif self.gamestate=="playing" and self.turn=="your" \
						and self.action_validation(s,character.player):
			...
			self.turn="opp"						# and sets the turn to "opponent"
						...
						and not self.action_validation(s,character.player):
	...
	def make_move(self,move,player):
		x=float(string.ascii_lowercase.index(move[0].lower()))	# get index of letter (A - 0)
		y=float(move[1:])										# convert to float 
		goban.store_ishi(x,y,player)							# store placed stones
		hx,hy=goban.make_hits(x,y,player)						# make 4 hits around ishi
		self.refresh_figure(x,y,hx,hy,player)					# and refresh figure
