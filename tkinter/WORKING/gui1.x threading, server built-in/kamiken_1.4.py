...
DONE16. Turn indication
	...
	18. pseudo-AI. Just choose any random valid square
...
#			if self.action_validation(msg[8:],player):
		...
		if re.search('^__CONF__',msg):
			self.send('confirmed move receive','conr')
			self.confirmed=True
...
#		self.text.bind('<1>',self.undisable_text)
		...
		self.button_AI = Tk.Button(master=self.root, text='Start AI game', 
									width='10', command=self.start_AI)
...
#				self.buttonsLOCAL()
	...
	def disconnect(self):
		...
		self.button_start.config(command=self.start, text='Join game', 
									state='normal')
		self.button_AI.configure(state='normal',command=self.start_AI)
		self.connected='no'
		character.player=0
		self.reset_board('finrestart')
...
#		self.draw_board(goban.size)						# draw board
	...
	def stop_AI(self):
		...
		self.button_server.configure(state='normal',command=self.start_server)
		self.button_start.configure(text="Join game",command=self.start)
		self.button_AI.configure(text='Start AI game',command=self.start_AI)
