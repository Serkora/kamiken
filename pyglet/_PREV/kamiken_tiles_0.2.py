...
'''
Окно, объявление batch'а и рисование label'а для сообщений
'''

window = pyglet.window.Window(width=BOARD_W*TILE_SIZE,
                              height=BOARD_H*TILE_SIZE + TILE_SIZE,
                              caption='Kamiken'                    )

batch = pyglet.graphics.Batch()
label = pyglet.text.Label(text=MSG, font_size=BOARD_W*2, font_name=FONT,
                              color=colors['b_stone'],
                              x = 0, y =BOARD_H*TILE_SIZE+5, batch = batch)
