...
class Board(pyglet.window.Window):
	...
	def on_mouse_press(self, x, y, button, modifiers):
		if button == mouse.LEFT:
			...
			if 1 <= y1 <= BOARD_H and 1 <= x1 <= BOARD_W:
				if ALL_STONES[y1-1,x1-1]%(self.player+2)==0 and self.turn==self.player:
					self.make_move(y1,x1,self.player)
					self.dispatch_event('on_mademove',x1,y1) # создаётся ивент, который 
	...
	def on_key_press(self, symbol, modifiers):
		...
		if symbol == key.P and modifiers and key.MOD_SHIFT:
